...
import { updateMemberRole } from '@/app/actions/events'
    ...
    <div className="min-h-screen bg-bg">
      ...
      <header className="sticky top-0 z-50 h-14 bg-bg2 border-b border-[rgba(232,184,75,0.20)] flex items-center justify-between px-6">
        <div className="flex items-center gap-3">
          <Link href={`/events/${params.id}`} className="text-text-2 hover:text-text text-sm transition-colors">← Board</Link>
          <span className="text-text-3">/</span>
          <span className="font-syne font-bold text-sm">Manage Event</span>
          <span className="font-pixel text-[6px] text-gold-dim">{event.name}</span>
        </div>
          ...
          <Card>
            <CardHeader className="flex items-center justify-between">
              ...
              {isOwner && (tiles ?? []).length === 0 && (
                <form action={async () => {
                  'use server'
                  await setBoardTemplate(params.id)
                }}>
            ...
            <CardBody className="p-4">
              ...
              <div className="grid gap-1 mb-4" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
                {Array.from({ length: 25 }, (_, pos) => {
                  ...
                  return (
                    <div
                      key={pos}
                      className={`aspect-square rounded border flex flex-col items-center justify-center p-1 relative text-center ${
                        tile
                          ? tile.free_space
                    ...
                    >
                      {tile ? (
                        <>
                          ...
                          />
                          <div className="font-pixel text-[4px] text-text-3 mt-0.5 overflow-hidden w-full text-center" style={{ textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {tile.free_space ? '★' : tile.name.split(' ')[0]}
                          </div>
                          {isOwner && (
                            <form action={async () => {
                              'use server'
                              await deleteTile(tile.id, params.id)
                            }} className="absolute top-0.5 right-0.5">
                              <button className="w-3.5 h-3.5 bg-[rgba(232,85,85,0.3)] hover:bg-[rgba(232,85,85,0.6)] rounded-sm text-[7px] text-red flex items-center justify-center transition-colors">
                                ×
                              </button>
                            </form>
                          )}
              ...
              {(tiles ?? []).length < 25 && (
                <form action={async (fd: FormData) => {
                  'use server'
                  await addTile(params.id, fd)
                }} className="space-y-3 pt-4 border-t border-[rgba(232,184,75,0.08)]">
                  ...
                  <div>
                    <label className="block text-[10px] text-text-3 mb-1">Sprite URL (optional — leave blank to use OSRS wiki)</label>
            ...
            <CardBody className="p-4 space-y-3">
              {(teams ?? []).map((team: any) => (
                <div key={team.id} className="flex items-center gap-3 py-2 px-3 rounded bg-bg3 border border-[rgba(255,255,255,0.04)]">
                  ...
                  {isOwner && (
                    <form action={async () => {
                      'use server'
                      await deleteTeam(team.id, params.id)
                    }}>
              ...
              {isOwner && (teams ?? []).length < 8 && (
                <form action={async (fd: FormData) => {
                  ...
                  if (name && color) await createTeam(params.id, name, color)
                }} className="space-y-2 pt-2 border-t border-[rgba(232,184,75,0.08)]">
                ...
                {(members ?? []).map((member: any) => {
                  ...
                  return (
                    <div key={member.id} className="flex items-center gap-3 px-4 py-3">
                      {/* Avatar */}
                      <div
                        className="w-8 h-8 rounded-full flex-shrink-0 border flex items-center justify-center font-pixel text-[7px]"
                        style={{
                      ...
                      {isOwner && member.role !== 'owner' && (
                        <div className="flex items-center gap-2">
                          <form action={async (fd: FormData) => {
                            ...
                            await assignMemberToTeam(member.id, tid, params.id)
                          }}>
                            <select
                              name="team_id"
                              ...
                              className="text-xs bg-surface2 border border-[rgba(232,184,75,0.12)] rounded px-2 py-1.5 text-text-2 outline-none hover:border-[rgba(232,184,75,0.25)] transition-colors cursor-pointer"
                            >
                          ...
                          <form action={async () => {
                            ...
                            await updateMemberRole(params.id, member.id, newRole)
                          }}>
                            <button
                              type="submit"
                              ...
                              title={member.role === 'moderator' ? 'Remove mod' : 'Make mod'}
                            >
            ...
            <Card>
              ...
              <CardBody className="p-4">
                <form action={async (fd: FormData) => {
                  ...
                  await db2.from('events').update({ discord_webhook_url: url || null }).eq('id', params.id)
                }} className="space-y-3">
                  <input
                    name="webhook_url"
                    type="url"
                  ...
                  />
                  <p className="text-xs text-text-3">When a tile is approved, TileScape will POST a rich embed to this channel.</p>
