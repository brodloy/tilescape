...
export async function generateMetadata({
  ...
  if (!searchParams.code) {
    return {
      title: 'Join Event — TileScape',
      description: 'Join an OSRS clan bingo event on TileScape.',
    }
  }
  ...
  if (!event) {
    return { title: 'Join Event — TileScape' }
  }
  ...
  const description = event.description
    ? event.description
    : `You've been invited to join "${event.name}" on TileScape — the OSRS clan bingo tracker.`
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: 'website',
      url: `${APP_URL}/join?code=${searchParams.code}`,
      siteName: 'TileScape',
      images: [
        {
          url: `${APP_URL}/api/og?code=${searchParams.code}`,
          width: 1200,
          height: 630,
          alt: title,
        },
          ...
          {eventPreview ? (
            <div className="p-6 border-b border-[rgba(232,184,75,0.10)]">
              ...
              <div className="flex items-center gap-2 mt-3">
                <span className={`font-pixel text-[9px] px-2 py-1 rounded border ${
